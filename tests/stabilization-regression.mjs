...
assert.equal(nonEmptyCount, 317, `Expected 317 boxable entries including forms; found ${nonEmptyCount}`);
assert.equal(new Set(ids).size, 317, 'Every boxable entry must have a unique data-id');
...
assert.ok(html.includes('#boxContainer .cell .checkbox'), 'The 4px name/checkmark refinement is missing');
assert.ok(html.includes('297-Normal') && html.includes('297-Black') && html.includes('297-White'), 'Kyurem form set is incomplete');
