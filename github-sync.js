(() => {
  ...
  const saveRemote = async () => {
    if(!token()||busy)return; busy=true; status("Token active • Saving to GitHub…","busy");
    try{
      ...
      if(result.content?.sha)localStorage.setItem(SHA_KEY,result.content.sha); status("Token active • Synced ✓","ok");
    }catch(e){
      ...
      else status(e.status===409?"Token active • GitHub changed; please reload before saving again.":`Sync failed: ${e.message}`,"error");
    }finally{busy=false;}
  };
  const scheduleSave=()=>{clearTimeout(timer);status("Token active • Unsaved changes…","busy");timer=setTimeout(saveRemote,1200);};
  const loadRemote = async () => {
    if(!token()){remoteReady=true;status("Local only — enter a GitHub token to sync.");return;}
    remoteReady=false; status("Token active • Loading from GitHub…","busy");
    try{
      ...
      suppressSave=false;
      remoteReady=true;
      status(`Token active • Synced ✓ (${remoteKeys.filter(k=>remote.state[k]).length} completed)`,"ok");
      ...
      if(e.status===401){localStorage.removeItem(TOKEN_KEY);localStorage.removeItem(SHA_KEY);setTokenUI(false);status("GitHub token is invalid or expired. Enter a new token.","error");}
      else status(e.status===404?"Token active • save.json not found; your next change will create it.":`Could not load GitHub save: ${e.message}`,e.status===401?"error":"normal");
    }
  };
  const connect = async () => {
  ...
  const injectUI=()=>{
    ...
    const style=document.createElement("style"); style.textContent=`#githubSyncPanel{max-width:900px;margin:0 auto 20px;padding:12px 14px;background:#fff;border:2px solid #cbd5e1;border-radius:12px;box-shadow:0 4px 6px -1px rgba(0,0,0,.05)}#githubSyncPanel summary{cursor:pointer;font-weight:700;color:#1e3a8a}#githubSyncControls{display:flex;gap:8px;flex-wrap:wrap;align-items:center;margin-top:10px}#githubTokenInput{flex:1;min-width:240px;padding:9px 10px;border:1px solid #cbd5e1;border-radius:7px;font:inherit}#githubTokenInput:disabled{background:#f1f5f9;color:#64748b;cursor:not-allowed}#githubSyncControls button{padding:9px 12px;border:1px solid #cbd5e1;border-radius:7px;background:#f8fafc;font-weight:600;cursor:pointer}#githubSyncStatus{margin-top:8px;font-size:.82rem;color:#64748b}#githubSyncStatus[data-type=ok]{color:#047857}#githubSyncStatus[data-type=error]{color:#b91c1c}#githubSyncStatus[data-type=busy]{color:#1d4ed8}#githubSyncNote{margin:8px 0 0;font-size:.76rem;color:#64748b;line-height:1.4}`; document.head.appendChild(style);
    const panel=document.createElement("details"); panel.id="githubSyncPanel"; panel.innerHTML=`<summary>GitHub Sync</summary><div id="githubSyncControls"><input id="githubTokenInput" type="password" autocomplete="off" placeholder="GitHub token"><button id="githubConnectBtn" type="button">Activate Token</button><button id="githubChangeTokenBtn" type="button" hidden>Change Token</button></div><div id="githubSyncStatus">Local only — enter a GitHub token to sync.</div><p id="githubSyncNote">Your token is stored only in this browser and is never committed to GitHub. When active, the token field is masked and locked so you can clearly see that GitHub sync remains enabled.</p>`;
    ...
    document.getElementById("githubChangeTokenBtn").onclick=()=>{clearTimeout(timer);localStorage.removeItem(TOKEN_KEY);localStorage.removeItem(SHA_KEY);remoteReady=false;setTokenUI(false);status("Enter a new GitHub token to activate sync.");};
