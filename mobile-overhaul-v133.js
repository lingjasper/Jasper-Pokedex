(() => {
  ...
  const installStyles = () => {
    ...
    style.textContent = `
      ...
      @media (max-width:640px) {
        ...
        #mobileHeader { position:fixed; top:0; left:0; right:0; width:auto; box-sizing:border-box; display:flex; flex-direction:column; align-items:stretch; justify-content:flex-start; gap:0; padding:8px 8px 0; margin:0!important; background:#fff; z-index:11900; box-shadow:0 1px 4px rgba(15,23,42,.08); }
  ...
  const installViewState = () => {
    ...
    const apply = () => {
      const b = document.getElementById('boxViewBtn'), l = document.getElementById('listViewBtn'), bx = document.getElementById('boxContainer'), ls = document.getElementById('listContainer');
      if (!b || !l || !bx || !ls) return;
      const list = window.JASPER_POKEDEX_VIEW === 'list';
      bx.style.display = list ? 'none' : 'flex'; ls.style.display = list ? 'block' : 'none'; b.classList.toggle('active', !list); l.classList.toggle('active', list);
      [b,l].forEach((button,i) => { const label = button.querySelector('span'); if (label) label.textContent = i===0 ? 'Box' : 'List'; button.setAttribute('aria-label', i===0 ? 'Box view' : 'List view'); button.title = i===0 ? 'Box view' : 'List view'; });
    };
    document.addEventListener('click', event => {
      const button = event.target.closest('#boxViewBtn,#listViewBtn');
      if (button) { window.JASPER_POKEDEX_VIEW = button.id === 'listViewBtn' ? 'list' : 'box'; setTimeout(apply,0); }
    ...
    window.JASPER_APPLY_VIEW_STATE = apply; apply();
  };

  const boot = () => { installStyles(); installHeader(); installProgress(); installControls(); installViewState(); updateMoniker(); syncHeaderHeight(); requestAnimationFrame(syncHeaderHeight); };
  const waitForEngine = () => { if (!isMobile()) return; if (window.JASPER_POKEDEX) boot(); else setTimeout(waitForEngine,50); };
  window.addEventListener('resize', () => { if (isMobile()) syncHeaderHeight(); });
