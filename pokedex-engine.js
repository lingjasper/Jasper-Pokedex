(() => {
  ...
  const ensureBanner=()=>{let b=document.getElementById('dexProgressBanner');if(!b){const w=document.getElementById('desktopWorkspace'),t=document.getElementById('desktopTableContainer');if(!w||!t)return null;b=document.createElement('div');b.id='dexProgressBanner';b.innerHTML='<span class="dex-progress-icon" data-icon="info-fill" data-icon-size="20"></span><span class="dex-progress-text"></span>';w.insertBefore(b,t);}return b;};
  const banner=(p=activePokemon,s=readState(activeGame))=>{const b=ensureBanner(),x=b?.querySelector('.dex-progress-text');if(!x)return;const done=p.reduce((n,q)=>n+(s[q.id]===true?1:0),0),total=p.length;x.textContent=`${done} of ${total} registered · ${total-done} remaining · ${Math.round(done/Math.max(1,total)*100)}% complete`;};
  ...
  const checkbox=(completed=false)=>`<span class="checkbox"><span data-icon="${completed?'check-fill':'empty-circle'}" data-icon-size="20"></span></span>`;
  const syncCheckboxIcons=()=>{const s=readState(activeGame);document.querySelectorAll('[data-id] .checkbox').forEach(box=>{const row=box.closest('[data-id]'),completed=!!row&&s[row.dataset.id]===true,icon=box.querySelector('.jasper-icon,[data-icon]');if(icon&&window.JASPER_ICONS)window.JASPER_ICONS.set(icon,completed?'check-fill':'empty-circle');});};
  const cell=(p,s)=>{const c=document.createElement('div');c.className=`cell${s[p.id]===true?' completed':''}`;c.dataset.id=p.id;c.dataset.num=p.num;c.dataset.name=p.name;c.innerHTML=`<span class="dex-num">${esc(p.num)}</span><span class="name">${esc(p.baseName)}${p.form?` <span class="form">${esc(p.form)}</span>`:''}</span>${checkbox(s[p.id]===true)}`;return c;};
  ...
  const views=()=>{const b=document.getElementById('boxViewBtn'),l=document.getElementById('listViewBtn'),bx=document.getElementById('boxContainer'),ls=document.getElementById('listContainer');if(!b||!l||!bx||!ls)return;b.innerHTML='<span data-icon="box-fill" data-icon-size="16"></span><span>Box view</span>';l.innerHTML='<span data-icon="list-fill" data-icon-size="16"></span><span>List view</span>';const box=()=>{bx.style.display='flex';ls.style.display='none';b.classList.add('active');l.classList.remove('active');};const listView=()=>{bx.style.display='none';ls.style.display='block';l.classList.add('active');b.classList.remove('active');};b.onclick=box;l.onclick=listView;box();};
  const update=id=>{const s=readState(activeGame);document.querySelectorAll(`[data-id="${CSS.escape(id)}"]`).forEach(e=>e.classList.toggle('completed',s[id]===true));syncCheckboxIcons();};
  ...
  const render=async g=>{
    ...
    const s=readState(g);boxes(p,s);list(p,s);views();installSearch();bind();ensurePageTitle();banner(p,s);counters();setMoniker();window.JASPER_ICONS?.hydrate(document);syncCheckboxIcons();
  };
